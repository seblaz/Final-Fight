...
EventoBossIA::EventoBossIA(Jugadores *jugadores, Entidad *entidad, int ciclo)  :
        entidad(entidad),
        jugadores(jugadores),
        ciclo(ciclo){}
    ...
    if ( mod <= 1000 ) {
        if( mod >= 400 ) {
            ...
            if( ciclo < 70 && modX >= 200){
                estado->caminar(restaPosicion.x <= 0, restaPosicion.x > 0, restaPosicion.y < 0,
                                restaPosicion.y > 0);
            }else if ( estadoDePersonaje->getEstado() == CAMINANDO && modX > 90 && modX < 200 && modY < 5 && ciclo < 75){
                estado->darGolpe();
            }else if ( ciclo < 100 ){
                estado->caminar(restaPosicion.x > 0, restaPosicion.x <= 0, restaPosicion.y > 0,
                                restaPosicion.y < 0);
            }else if ( estadoDePersonaje->getEstado() == CAMINANDO && ciclo < 105){
                estado->saltar();
            }else if ( ciclo < 180 && modX >= 200){
                estado->caminar(restaPosicion.x <= 0, restaPosicion.x > 0, restaPosicion.y < 0,
                                restaPosicion.y > 0);
            }else if ( estadoDePersonaje->getEstado() == CAMINANDO && modX > 90 && modX < 200 && modY < 5 && ciclo < 185) {
                estado->darGolpe();
            }else{
                estado->caminar(restaPosicion.x > 0, restaPosicion.x <= 0, restaPosicion.y > 0,
                                restaPosicion.y < 0);
