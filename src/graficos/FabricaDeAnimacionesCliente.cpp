...
FabricaDeAnimacionesCliente::FabricaDeAnimacionesCliente (string rutaBase):rutaBase (std::move
	  (rutaBase))
{
}

AnimacionCliente *
FabricaDeAnimacionesCliente::getAnimacion (const string & rutaFinal)
{
  if (animaciones.find (rutaFinal) == animaciones.end ())
	...
	{
	  string rutaAux =
	    rutaBase + rutaFinal + "/posiciones/posicion" + to_string (i);
	  SDL_Rect posicion;
  ...
  return new AnimacionCliente (get < 0 > (animaciones[rutaFinal]),
			       get < 1 > (animaciones[rutaFinal]));
}

AnimacionCliente *
FabricaDeAnimacionesCliente::getAnimacion (ESTADO_DE_PERSONAJE estado)
{
