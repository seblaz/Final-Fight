...
void
GraficoJugador::renderizarIndicadorDeJugador ()
{
  ...
  Posicion nuevaPosicionIndicador (posicion->getX () - posicionDeEscenarioX,
				   posicion->getY (),
				   posicion->getZ () + 450);
  SDL_Rect posicionEnPantallaIndicador =
    calcularPosicionEnPantalla (nuevaPosicionIndicador,
				posicionEnSpriteIndicador,
				1.5);
  SDL_RenderCopy (renderer, spriteIndicador->getTexture (),
		  &posicionEnSpriteIndicador, &posicionEnPantallaIndicador);


  SDL_Rect rect =
    { 5 + (numeroJugador->numeroJugador - 1) * 350, 40, 20, 50 };
...
void
GraficoJugador::renderizarVidaDeJugador ()
{

  SDL_Renderer *renderer = Locator::renderer ();
  auto *spriteVida = entidad->getEstado < Sprite > ("sprite vida");
  auto *spriteVidaActual =
    entidad->getEstado < Sprite > ("sprite vida actual");
  auto *spriteCorazon =
    entidad->getEstado < Sprite > ("sprite cantidad de vidas");
  ...
  SDL_Rect posicionEnPantallaVidaLlena =
    { 30 + (numeroJugador->numeroJugador - 1) * 350, 40, 95 * escalaVida,
    17 * escalaVida
  };


  SDL_RenderCopy (renderer, spriteVida->getTexture (),
		  &posicionEnSpriteVidaLlena, &posicionEnPantallaVidaLlena);

  SDL_Rect posicionEnSpriteVidaActual =
    { 0, 0, (82 * energia->getEnergia () / 100), 9 };
  SDL_Rect posicionEnPantallaVidaActual =
    { 69 + (numeroJugador->numeroJugador - 1) * 350, 64,
  ...
  SDL_RenderCopy (renderer, spriteVidaActual->getTexture (),
		  &posicionEnSpriteVidaActual, &posicionEnPantallaVidaActual);
  ...
  for (int i = 0; i < cantidadDeVidasActuales; i++)
    {
      SDL_Rect posicionEnPantallaCorazon =
	{ 230 + (i * 26) + (numeroJugador->numeroJugador - 1) * 350, 40,
	11 * 2,
	11 * 2
      };
      SDL_RenderCopy (renderer, spriteCorazon->getTexture (),
		      &posicionEnSpriteCorazon, &posicionEnPantallaCorazon);
    }
}

void
GraficoJugador::renderizarPuntosDeJugador (Puntaje * puntaje,
					   NumeroJugador * numeroJugador)
{
  ...
  SDL_Surface *surfacePuntos = TTF_RenderText_Solid (Locator::fuente (),
						     puntos.c_str (),
						     colorDeFuente);
  SDL_Texture *texturaPuntos =
    SDL_CreateTextureFromSurface (renderer, surfacePuntos);
  SDL_FreeSurface (surfacePuntos);

  SDL_Rect posicionPuntos =
    { 40 + (numeroJugador->numeroJugador - 1) * 350, 0, 25 * cantidadDigitos,
    45
  };
