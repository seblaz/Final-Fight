...
EstadoDePersonajeServidor::EstadoDePersonajeServidor (Entidad * entidad):EstadoDeColisionable
  (entidad)
{
  auto *
    estadoDePersonaje =
  ...
  string
    estado =
    estadoDePersonaje ? EstadoDePersonaje::estadoACadena (estadoDePersonaje->
							  getEstado ()) :
    "reposando";
  auto *
    personaje = entidad->getEstado < Personaje > ("personaje");
  string
    nombrePersonaje =
    estadoDePersonaje ? Personaje::PersonajeACadena (personaje->
						     getPersonaje ()) :
    "cody";
  framesFaltantes =
    Locator::configuracion ()->getIntValue ("/animaciones/" +
					    nombrePersonaje + "/" + estado +
					    "/total");
}

template < typename T >
  EstadoDePersonajeServidor * crearEstado (Entidad * entidad)
{
  return new T (entidad);
}

map < ESTADO_DE_PERSONAJE,
  EstadoDePersonajeServidor * (*)(Entidad * entidad) >
  EstadoDePersonajeServidor::mapa = {
...
void
EstadoDePersonajeServidor::cambiarEstado (ESTADO_DE_PERSONAJE estado)
{
  enum PERSONAJE personaje =
    entidad->getEstado < Personaje > ("personaje")->getPersonaje ();
  auto *animacion =
    FabricaDeAnimacionesServidor::getAnimacion (entidad, personaje, estado);
  entidad->agregarComportamiento ("animacion servidor", animacion);
  entidad->getEstado < IndiceSprite > ("indice sprite")->setIndice (0);

  entidad->agregarEstado ("estado de personaje",
			  new EstadoDePersonaje (estado));
  entidad->agregarComportamiento ("estado", mapa[estado] (entidad));
}

void
EstadoDePersonajeServidor::saltar ()
{
...
void
EstadoDePersonajeServidor::agachar ()
{
  cambiarEstado (AGACHADO);
}

void
EstadoDePersonajeServidor::caminar (bool X_pos, bool X_neg, bool Y_pos,
				    bool Y_neg)
{
...
void
EstadoDePersonajeServidor::darGolpe ()
{
  cambiarEstado (DANDO_GOLPE);
}

void
EstadoDePersonajeServidor::recibirGolpeDe (Entidad * golpeador)
{
  ...
  golpeador->getEstado < NotificadorDeGolpes >
    ("notificador")->notificarGolpeAPersonaje (this->entidad);
  arma->inicioUso ();
...
void
EstadoDePersonajeServidor::morir ()
{
  if (!entidad->getEstado < Energia > ("energia")->vivo ())
    Locator::mapa ()->quitarPersonaje (entidad);
