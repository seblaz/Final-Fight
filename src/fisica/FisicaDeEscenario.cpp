...
FisicaDeEscenario::actualizar ()
{
  ...
  if (xMayorPersonaje - xMenorPersonaje <=
      ancho - scrollDerecho - scrollIzquierdo
      && xMenorPersonaje >= xScrollIzquierdo && enemigosAtacando < 2)
    {
      if ((xMayorPersonaje - xEscenario > ancho - scrollDerecho)
	  && (largo - xMayorPersonaje) > scrollDerecho)
	{
      ...
      if ((xMenorPersonaje - xEscenario < scrollIzquierdo)
	  && (xMenorPersonaje - scrollIzquierdo) > 0)
	{
  ...
  if (xMayorPersonaje > largo - 100 && enemigosAtacando == 0)
      ...
      if (xMayorPersonaje > largo)
	{
	  ...
	  Locator::logger ()->log (INFO,
				   "Se llego al final del " +
				   nivel->nivel () + " .");
	  mapa->vaciarMapa ();
	  string proxEtapa =
	    nivel->nivel () == "nivel1" ? "puntuacion1" : "puntuacion2";
