...
void
PantallaError::graficar (SDL_Renderer * renderer)
{
  int ancho = Locator::configuracion ()->getIntValue ("/resolucion/ancho");
  int alto = Locator::configuracion ()->getIntValue ("/resolucion/alto");

  auto *spritePantallaIngreso =
    Locator::fabricaDeSprites ()->getSpriteConfigPath (configPath);

  SDL_Rect posicionEnPantallaIngreso = { 0, 0, ancho, alto };
  SDL_RenderCopy (renderer, spritePantallaIngreso->getTexture (), nullptr,
		  &posicionEnPantallaIngreso);
}
