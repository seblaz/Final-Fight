...
disponible (0), finSemaforo (0)
{
}

void
ReceptorCliente::recibir ()
{
  stringstream s;
  while (!fin && !Locator::socket ()->estaDesconectado ())
      ...
      if (!Locator::socket ()->recibir (s))
	{
	  Locator::logger ()->log (ERROR,
				   "Se detecta desconexión del servidor.");
...
void
ReceptorCliente::devolverStreamMasReciente (stringstream & s)
{
  disponible.wait ();
  {
    std::lock_guard < std::mutex > lock (mutex);
