...
void
VistaPuntuacion::generarFondo (SDL_Renderer * renderer)
{
  ...
  auto *fondo =
    ...
    ("/pantallas/puntuacion/fondo/src");
...
void
VistaPuntuacion::generarTitulo (SDL_Renderer * renderer)
{
  Configuracion *config = Locator::configuracion ();
  int ancho = config->getIntValue ("/resolucion/ancho");
  ...
  SDL_Surface *surfaceTexto =
    TTF_RenderText_Solid (Locator::fuente (), titulo.c_str (), white);
...
void
VistaPuntuacion::generarPuntuaciones (SDL_Renderer * renderer)
{

  // Ordenar por puntuación más alta.
  auto puntos = puntuaciones->getPuntuaciones ();
  std::vector < std::pair < std::string, int >>puntosOrdenados
  {
  std::make_move_iterator (begin (puntos)),
      std::make_move_iterator (end (puntos))};
  sort (begin (puntosOrdenados), end (puntosOrdenados),[](auto p1, auto p2)
	{
...
for (const auto & tupla:puntosOrdenados)
    {
      SDL_Surface * surfaceUsuario =
	TTF_RenderText_Solid (Locator::fuente (), tupla.first.c_str (),
			      white);
      ...
      SDL_Texture *texturaUsuario =
	SDL_CreateTextureFromSurface (renderer, surfaceUsuario);
...
void
VistaPuntuacion::graficar (SDL_Renderer * renderer)
{
...
string
VistaPuntuacion::getConfigPathMusica ()
{
  return "/pantallas/puntuacion/musica/src";
}

int
VistaPuntuacion::getLoops ()
{
  return 1;
}
