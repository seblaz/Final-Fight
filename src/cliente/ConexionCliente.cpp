  ...
  if (descriptorSocket == -1)
    {
      Locator::logger ()->log (ERROR,
			       "No se pudo crear el descriptor de socket: " +
			       string (strerror (errno)) + ".");
      exit (0);
    }

  int status = connect (descriptorSocket, (sockaddr *) & sendSockAddr,
			sizeof (sendSockAddr));
  if (status < 0)
    {
      Locator::logger ()->log (ERROR,
			       "El cliente no pudo conectarse al servidor: " +
			       string (strerror (errno)) + ".");
      exit (0);
    }
  else
    {
      Locator::logger ()->log (INFO,
			       "El cliente se conectó al servidor correctamente.");
    }
