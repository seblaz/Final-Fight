...
Animador::actualizar ()
{
  string nombrePersonaje =
    Personaje::PersonajeACadena (entidad->getEstado < Personaje >
				 ("personaje")->getPersonaje ());
  string tipoArma =
    Arma::armaACadena (entidad->getEstado < Arma > ("arma")->getArma ());
  auto *spritePersonaje =
  ...
  entidad->agregarEstado ("animacion",
			  fabricaDeAnimaciones->getAnimacion (estado->
							      getEstado ()));
}
