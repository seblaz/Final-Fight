...
void
Logger::log (LEVEL level_, const string & message)
{
  if (this->level >= level_)
    {
      string log =
	getLogDateTime () + " " + levelToString (level_) + " " + message;
      printf ("%s\n", log.c_str ());
...
LEVEL
Logger::stringToLevel (const string & stringLevel)
{
...
void
Logger::setLevel (const string & newLevel)
{
  if (newLevel != "DEBUG" && newLevel != "INFO" && newLevel != "ERROR")
    {
      log (ERROR,
	   "Nivel de logger invalido: " + newLevel +
	   ". Se mantiene el nivel de log en DEBUG.");
    }
  else
    {
