...
bool
Socket::enviarSinChequeo (stringstream & s)
{
  string msg = s.str ();
  size_t total = 0;
  ...
  do
    {
      enviados =
	send (socket, msg.c_str () + total, tamano - total,
  ...
  if (enviados == -1)
    {
      Locator::logger ()->log (ERROR,
			       "Ocurrió un error al enviar por el socket: " +
...
bool
Socket::recibirConCantidad (stringstream & s, size_t cantidad)
{
  ...
  if (leidos == -1)
    {
      Locator::logger ()->log (ERROR,
			       "Ocurrió un error al recibir por el socket: "
			       + string (strerror (errno)));
      return false;
    }
  return leidos != 0;
...
Socket::finalizarConexion ()
{
  Locator::logger ()->log (INFO,
			   "Se termina la conexión a través del socket.");
