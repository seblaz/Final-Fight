...
void
Energia::serializar (ostream & stream)
{
...
void
Energia::deserializar (istream & stream)
{
...
void
Energia::restarEnergia (int energiaRestada)
{
  Locator::logger ()->log (DEBUG,
			   "Se resta puntos de vida: " +
			   to_string (energiaRestada));
  ...
  if (vidas-- > 0)
    {
      ...
      Locator::logger ()->log (DEBUG,
			       "Quedan puntosDeEnergia: " +
			       to_string (puntosDeEnergia));
      return true;
    }
  return false;
}
