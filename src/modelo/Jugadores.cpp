...
void
Jugadores::agregarJugador (IdEntidad id, Entidad * jugador)
{
...
void
Jugadores::reiniciarPosiciones (int x, int y)
{
for (auto tuple:jugadores)
    {
      if (tuple.second->getEstado < Energia > ("energia")->vivo ())
...
void
Jugadores::bloquearMovimientos (int scrollIzquierdo, int scrollDerecho)
{
for (auto tuple:jugadores)
    {
      if (tuple.second->getEstado < Energia > ("energia")->vivo ())
	{
	  ...
	  if (x >= scrollDerecho)
	    {
	      tuple.second->getEstado < Posicion > ("posicion")->x =
		scrollDerecho;
	    }
...
void
Jugadores::arrastrarInactivos (int scrollIzquierdo, int scrollDerecho)
{
for (auto tuple:jugadores)
    {
...
Posicion
Jugadores::posicionMasCercana (Posicion * posicion)
{
...
for (auto tuple:jugadores)
    {
	  ...
	  tuple.second->getEstado < Energia > ("energia")->vivo ())
	{
	  auto *posicionJugador =
	    tuple.second->getEstado < Posicion > ("posicion");
	  int
	    distJugadorYenemigo =
	    posicionJugador->distanciaEntrePuntos (posicion);
	  int
	    distMasCercanaActual =
	    posicionMasCercana.distanciaEntrePuntos (posicion);
