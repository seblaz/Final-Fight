...
AnimacionServidor *
FabricaDeAnimacionesServidor::getAnimacion (Entidad * entidad,
					    const string & ruta)
{
  Configuracion *config = Locator::configuracion ();
  vector < int >duraciones =
    split (config->getValue ("/animaciones" + ruta + "/duraciones"), ",");
  return new AnimacionServidor (entidad, duraciones,
				config->getIntValue ("/animaciones" + ruta +
						     "/total"));
}
...
{
  return getAnimacion (entidad, personajeSeleccionado,
		       EstadoDePersonaje::estadoACadena (estado));
}
