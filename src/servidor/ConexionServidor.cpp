  ...
  if (descriptorSocket < 0)
    {
      Locator::logger ()->log (ERROR,
			       "No se pudo crear el socket para aceptar clientes en el servidor. Se termina el programa.");
      exit (0);
    }

  int enable = 1;
  if (setsockopt
      (descriptorSocket, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof (int)) < 0)
    {
  ...
  if (bindStatus < 0)
    {
      Locator::logger ()->log (ERROR,
			       string
			       ("No se pudo realizar el binding en el socket del servidor. Error: ").
