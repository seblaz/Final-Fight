...
Entidad *
NivelServidor::generarJugador (Mapa * mapa,
			       ...
			       int contadorJugador)
{
  ...
  auto *animacionServidor =
    FabricaDeAnimacionesServidor::getAnimacion (jugador,
						personajeSeleccionado,
						"reposando");
  auto *energia = new Energia (100, 2);
  auto *envolvente = new EnvolventeVolumen (posicion, 50, 15);
  auto *envolventeDeAtaque =
    new EnvolventeAtaque (posicion, 100, 30, orientacion);
...
void
NivelServidor::generarNivel (const string & nivel, Mapa * mapa)
{
  ...
  if (nivel == "nivel2")
    generarJefeFinal (nivel, mapa, jugadores);
}

Entidad *
NivelServidor::generarEscenario (const string & nivel, Mapa * mapa)
{
  ...
  int profundidad =
    config->getIntValue ("/niveles/" + nivel + "/escenario/profundidad");
  int anchoNivel =
  ...
  Locator::logger ()->log (DEBUG,
			   "Se cargo profundidad para escenario: " +
...
void
NivelServidor::generarTransicion (const string & nivel, Mapa * mapa,
				  Jugadores * posicionDeJugadores)
{

  Entidad *transicion = mapa->crearTransicion ();
  auto *tipo = new Tipo (TIPO::TRANSICION);
  int anchoDeNivel =
    Locator::configuracion ()->getIntValue ("/niveles/" + nivel +
					    "/escenario/ancho");
  auto *posicion = new Posicion (0, 1, 0);
  auto *opacidad = new Opacidad ();
  auto *fisicaDeTransicion =
    new FisicaDeTransicion (transicion, anchoDeNivel);
  ...
  transicion->agregarComportamiento ("fisica de transicion",
				     fisicaDeTransicion);
}

int
generarPosicionX (int frontera)
{
  random_device rd;
  mt19937 mt (rd ());
...
void
NivelServidor::generarEnemigo (const string & nivel, Mapa * mapa,
			       Jugadores * jugadores)
{
  ...
  int cantidad =
    config->getIntValue ("/niveles/" + nivel +
			 "/escenario/enemigos/cantidad");
  int anchoNivel =
    config->getIntValue ("/niveles/" + nivel + "/escenario/ancho");
  int profundidadNivel =
    config->getIntValue ("/niveles/" + nivel + "/escenario/profundidad");
  ...
  for (int i = 0; i < cantidad; i++)
    {
      ...
      auto *estadoDePersonaje = new EstadoDePersonaje (REPOSANDO);
      auto *posicionEnemigoRandom =
	new Posicion (generarPosicionX (anchoNivel),
		      generarPosicionY (profundidadNivel), 0);
      auto *indiceSprite = new IndiceSprite;
      auto *animacionServidor =
	FabricaDeAnimacionesServidor::getAnimacion (enemigo, POISSON,
						    "reposando");
      auto *energia = new Energia (100, 0);
      auto *envolvente =
	new EnvolventeVolumen (posicionEnemigoRandom, 50, 30);
      auto *envolventeDeAtaque =
	new EnvolventeAtaque (posicionEnemigoRandom, 100, 30,
			      orientacionDeEnemigo);
      auto *arma = new Arma (ARMA::PUNIOS);
      ...
      enemigo->agregarComportamiento ("animacion servidor",
				      animacionServidor);
    }
}

void
NivelServidor::generarElementos (const string & nivel, Mapa * mapa,
				 ELEMENTO objeto)
{
  Configuracion *config = Locator::configuracion ();

  int cantidad =
    config->getIntValue ("/niveles/" + nivel + "/escenario/elementos/" +
			 Elemento::ElementoACadena (objeto) + "/cantidad");
  int anchoNivel =
    config->getIntValue ("/niveles/" + nivel + "/escenario/ancho");
  int profundidadNivel =
    config->getIntValue ("/niveles/" + nivel + "/escenario/profundidad");
  ...
  for (int i = 1; i <= cantidad; i++)
    {
      Locator::logger ()->log (INFO,
			       "Se inicia la construccion del elemento random :"
			       + to_string (i));
      auto *indiceSprite = new IndiceSprite;
      ...
      auto *posicionElementoRandom =
	new Posicion (generarPosicionX (anchoNivel),
		      generarPosicionY (profundidadNivel),
		      0);
      auto *envolvente =
	new EnvolventeVolumen (posicionElementoRandom, 75, 8);
      auto *velocidad = new Velocidad ();
...
void
NivelServidor::generarArmas (const string & nivel, Mapa * mapa, ARMA tipoArma)
{
  Configuracion *config = Locator::configuracion ();
  int cantidad =
    config->getIntValue ("/niveles/" + nivel + "/escenario/armas/" +
			 Arma::armaACadena (tipoArma) + "/cantidad");
  int anchoNivel =
    config->getIntValue ("/niveles/" + nivel + "/escenario/ancho");
  int profundidadNivel =
    config->getIntValue ("/niveles/" + nivel + "/escenario/profundidad");
  ...
  for (int i = 1; i <= cantidad; i++)
	...
	new Posicion (generarPosicionX (anchoNivel),
		      generarPosicionY (profundidadNivel), 0);
      auto *envolvente =
	new EnvolventeVolumen (posicionElementoRandom, 20, 8);
...
void
NivelServidor::generarJefeFinal (const string & nivel, Mapa * mapa,
				 Jugadores * jugadores)
{
  Configuracion *config = Locator::configuracion ();
  int anchoNivel =
  ...
  auto *estadoDePersonaje = new EstadoDePersonaje (REPOSANDO);
  auto *posicionEnemigoRandom =
    new Posicion (anchoNivel - 100, generarPosicionY (profundidadNivel), 0);
  auto *indiceSprite = new IndiceSprite;
  auto *animacionServidor =
    FabricaDeAnimacionesServidor::getAnimacion (enemigo, BOSS, "reposando");
  auto *energia = new Energia (500, 0);
  auto *envolvente = new EnvolventeVolumen (posicionEnemigoRandom, 100, 30);
  auto *arma = new Arma (ARMA::PUNIOS);
  auto *envolventeDeAtaque =
    new EnvolventeAtaque (posicionEnemigoRandom, 200, 30,
			  orientacionDeEnemigo);
  auto *notificador = new NotificadorDeGolpes ();
