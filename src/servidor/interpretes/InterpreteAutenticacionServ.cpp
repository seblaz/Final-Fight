...
usuarioAgregado (0),
InterpreteServidor (usuario), autenticacion (autenticacion)
{
}

bool
InterpreteAutenticacionServ::interpretarAccion (ACCION accion,
						stringstream & s)
{
  if (accion == ENVIAR_USUARIO)
    {
      getUsuario ()->deserializar (s);
      if (!validarContrasenia (getUsuario ()))
	return true;

      auto *crear =
	new AgregarUsuario (getUsuario (), autenticacion, usuarioAgregado,
			    getEtapa ()->getManager ());
      Locator::eventos ()->push (crear);
      usuarioAgregado.wait ();

      if (!getUsuario ()->getValido ())
	{			// Usuario invalido
	  Locator::logger ()->log (INFO,
				   "Se desconecta un cliente que no pasó la etapa de autenticación.");
	  return false;
	}
  ...
  else
    {
      Locator::logger ()->log (ERROR,
			       "Se recibió una acción inválida en el actualizador de usuario.");
    }
  return false;
}

bool
InterpreteAutenticacionServ::validarContrasenia (Usuario * usuario)
{
  if (Locator::usuarios ()->validarContrasenia (usuario))
    {
      Locator::logger ()->log (INFO,
			       "Se recibió una contraseña correcta del usuario: "
			       + usuario->getUsuario () + ".");
      return true;
    }

  Locator::logger ()->log (ERROR,
			   "Se recibió una contraseña incorrecta del usuario: "
			   + usuario->getUsuario () + ".");
  autenticacion->passwordIncorrecta ();
  return false;
}

AgregarUsuario::AgregarUsuario (Usuario * usuario, ModeloAutenticacion * autenticacion, semaphore & semaphore, ManagerEtapas * etapas):
etapas (etapas),
usuario (usuario), usuarioAgregado (semaphore), autenticacion (autenticacion)
{
}

void
AgregarUsuario::resolver ()
{
  ...
  if (manager->estaPresente (usuario))
    {
      ...
      if (usuarioAnterior->estaConectado ())
	{			// El usuario esta conectado desde otro cliente.
	  usuario->setValido (false);
	  Locator::logger ()->log (ERROR,
				   "El usuario: " +
				   usuarioAnterior->getUsuario () +
				   " se encuentra conectado desdo otro cliente.");
	  autenticacion->usuarioYaConectado ();

	}
      else
	{			// El usuario se había desconectado.
	...
	{			// No queda espacio en la partida
	  Locator::logger ()->log (ERROR,
				   "El usuario: " + usuario->getUsuario () +
