...
bool
InterpreteMenuSeleccionServ::interpretarAccion (ACCION accion,
						stringstream & s)
{
  auto *evento = new ConfirmarSeleccion (getUsuario (),
					 static_cast <
					 enum PERSONAJE >(accion -
							  SELECCIONAR_GUY));
...
void
InterpreteMenuSeleccionServ::finalizarCliente ()
{
  getUsuario ()->desconectar ();
  getUsuario ()->setPersonajeSeleccionado (NULO);
}

ConfirmarSeleccion::ConfirmarSeleccion (Usuario * usuario,
...
for (auto * iUsuario:Locator::usuarios ()->getUsuarios ())
  ...
  if (listos && !Locator::usuarios ()->faltanJugadores ())
	...
	{
	  Entidad *personaje =
	    NivelServidor::generarJugador (Locator::mapa (),
					   iUsuario->getPersonajeSeleccionado
					   (),
					   numeroJugador);
