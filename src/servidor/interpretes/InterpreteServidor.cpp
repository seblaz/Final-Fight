...
bool
InterpreteServidor::interpretarStream (stringstream & s)
{
  Accion accion;
...
Usuario *
InterpreteServidor::getUsuario ()
{
  return usuario;
}

void
InterpreteServidor::desconectarUsuarioVoluntariamente ()
{
  if (usuario)
    {
      Locator::logger ()->log (INFO,
			       "Se desconecta voluntariamente el usuario " +
			       usuario->getUsuario () + ".");
...
void
InterpreteServidor::setEtapa (Etapa * etapa_)
{
  etapa = etapa_;
}

Etapa *
InterpreteServidor::getEtapa ()
{
  return etapa;
}
...
void
DesconectarVoluntariamente::resolver ()
{
