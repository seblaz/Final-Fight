...
void
ConexionesClientes::manejarConexiones ()
{
  while (true)
    {
      ...
      int nuevoSocket =
	accept (socketServidor->getIntSocket (), (sockaddr *) & newSockAddr,
		&newSockAddrSize);
      if (nuevoSocket < 0)
	{
	  Locator::logger ()->log (ERROR,
				   "Error al aceptar el pedido de conexión del cliente.");
	  break;
	}
      else
	{
...
pthread_t
ConexionesClientes::manejarConexionesEnHilo ()
{
  Locator::logger ()->log (DEBUG,
			   "Se creó el hilo de manejo de conexiones.");
