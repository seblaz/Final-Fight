...
void
configApplication (int argc, const char *args[])
{
  ...
  Configuracion *config =
    ...
    new Configuracion () :
    (argc == 2 ? new Configuracion (args[1]) : new Configuracion (args[2]));
  ...
  if (!defaultLogger)
    {
      string newLevel = (argc > 2
			 || paramIsLoggerLevel) ? args[1] : loggerLevel;
  ...
  Locator::logger ()->log (INFO,
			   "Archivo de configuracion abierto: " + configPath);
}
...
main (int argc, const char **args)
{
  ...
  Locator::provide (eventosAProcesar);
     ...
     */
  int maximoJugadores =
    Locator::configuracion ()->getIntValue ("/jugadores/cantidad");
  ManagerUsuarios managerUsuarios (maximoJugadores);
  Locator::provide (&managerUsuarios);
  Locator::logger ()->log (INFO,
			   "Esperando " + to_string (maximoJugadores) +
			   " jugador(es).");
  ...
  {
    Locator::logger ()->log (ERROR,
			     "Ocurrió una excepción y se debió finalizar el programa.");
    exit (0);
  }
